...
data = dict(
    ...
    description="Allocatoin Prolbem",
...
def packages():
    return ["pyallocation"] + ["pyallocation." + e for e in setuptools.find_packages(where='pyallocation')]
...
def run_setup(setup_args):
    ...
    try:
        ...
        kwargs['ext_modules'] = []
        cython_folder = os.path.join("pyallocation", "cython")
        cython_files = os.listdir(cython_folder)

        # if the pyx files should be translated and then compiled
        if do_cythonize:
            from Cython.Build import cythonize
            kwargs['ext_modules'] = cythonize("pyallocation/cython/*.pyx")
